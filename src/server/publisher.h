...
template <size_t ConfigIndex>
Publisher<ConfigIndex>::Publisher(zmq::context_t &ctx): pub{ctx, ZMQ_PUB} {
    pub.bind(std::string{ConfigList[ConfigIndex].port});
}

template <size_t ConfigIndex>
template <typename T>
zmq::recv_result_t Publisher<ConfigIndex>::send(const std::array<T, ConfigList[ConfigIndex].Nchannels> &arr) {
